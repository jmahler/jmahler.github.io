...
The following two fixes help get suspend working, but it isn't perfect.
First is to add the force option to the tpm_tis module.  This can be
added to a modprobe.d .conf file [[8]].

    # /etc/modprobe.d/tpm_tis.conf
    options tpm_tis force=1

The second fix is to unload the ehci-pci driver before suspending since
it does not handle suspend/resume properly.  The following script can
placed in to `/etc/pm/sleep.d`.

    #!/bin/sh
    
    # The ehci-pci driver has problems when suspending on a Acer C720
    # so unload the driver to get around the issue.
...
With this configuration it will "work" but there will be lots of error
messages in the kern.log.

    /var/log/kern.log
    ...
    Jul 29 21:30:36 newt kernel: [  228.622696]  0000000000000000
    ffffffff81064ee7 ffffffffa05b0c10 0000000000000018
    Jul 29 21:30:36 newt kernel: [  228.622697] Call Trace:
    Jul 29 21:30:36 newt kernel: [  228.622703]  [<ffffffff814ea9fa>] ?
    dump_stack+0x41/0x51
    Jul 29 21:30:36 newt kernel: [  228.622707]  [<ffffffff81064e82>] ?
