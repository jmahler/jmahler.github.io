...
A downside of using the config provided by a distribution is that it
will take a very long time to build because lots of modules are
included.  They are meant to support a lot of different systems however
most of them are unnecessary for one specific system.  One way to reduce
the configuration to just the set of modules needed by a system is to
use `localmodconfig` [[9]].
...
`localmodconfig` will only remove modules, it will not add them.  This
means the distributions config, which has lots of modules, makes a good
starting point.
