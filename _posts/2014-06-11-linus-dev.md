...
Applying Patches from Mutt
==========================

Patches are communicated in emails sent through the mailing lists.
They are then downloaded and applied to the kernel to be tested.

If there is just a single email it can be saved (`s`) and then
applied using `git am`.

If there is a thread of emails these can be saved to one directory
by tagging them first.

    <Esc>t      # tag an entire thread
    t           # toggle tagging of a message
    ;s          # save tagged messages
    u           # un-delete a message
